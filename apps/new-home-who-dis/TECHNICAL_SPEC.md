...
- **Deployment**: Railway
- **Domain**: party.razsbg.top (single subdomain with `/tv-display` route)
...
- **Railway Advantage**: Persistent connections with no cold starts or execution time limits
- **Long-running Support**: Railway's infrastructure ideal for SSE connections during live party

**Railway Benefits for Real-time:**
- ✅ No 10-second execution limit (unlike serverless)
- ✅ No cold starts affecting connection stability
- ✅ Persistent server instances maintain WebSocket/SSE connections
- ✅ Better suited for live leaderboard updates during party hours
...
**Note:** Railway automatically generates and manages `DATABASE_URL` when you add PostgreSQL to your project.

#### Railway Configuration

Railway uses a simple configuration approach:

- **Service Type**: Web service
- **Build Command**: `pnpm install && pnpm build`
- **Start Command**: `pnpm start` or custom server
- **PostgreSQL**: Add via `railway add` (auto-configures DATABASE_URL)
- **Domain**: Configure via Railway dashboard or CLI
- **No timeout limits**: Long-running SSE connections fully supported
